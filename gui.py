...
class FonemarikApp(QWidget):
    ...
    def initUI(self):
        ...
        self.query_input.setPlaceholderText("Введите звук, слог или фонемный шаблон")

        self.position_combo = QComboBox(self)
        self.position_combo.addItems(["В любом месте", "В начале", "В середине", "В конце"])
        ...
        self.frequency_sort_checkbox.setToolTip("Сортировать результаты по убыванию частоты встречаемости слова")

        self.export_button = QPushButton("Экспорт в HTML", self) # Обновленный текст кнопки
    ...
    def update_stress_sounds(self):
        ...
        try:
            ...
            current_items = {self.stress_sound_combo.itemText(i) for i in range(self.stress_sound_combo.count())}
            for sound in sounds:
        ...
        try:
            ...
            if self.search_results:
                ...
                for row in self.search_results:
                    ...
                    stressed_word = format_word_with_stress(row['word'], row.get('stress_position'))
                    row_info = f"Слово: {stressed_word}{frequency_info}\nТранскрипция: {row['transcription_cyrillic']}{stress_info}\nЧасть речи: {row.get('part_of_speech', 'N/A')}\n\n"
